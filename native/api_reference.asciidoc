...
[source, python]
.5. Subscribing with State (You can set state for a uuid during a subscribe by using the state parameter. Simply specify the desired key/value pairs as a JSON object.):
----
...
[source, python]
----
pubnub.set_state().channels(str|list|tuple).channel_groups(str|list|tuple).state(dict).sync()
----
...
[source, python]
----
pubnub.set_state().channels(str|list|tuple).channel_groups(str|list|tuple).uuid(str).sync()
----
...
----
my_state = {'age': 20}
envelope = pubnub.set_state().channels(['ch1', 'ch2', 'ch3']).state(my_state).sync()
----
...
----
my_state = {'age': 20}
envelope = pubnub.set_state().channel_gorups(['gr1', 'gr2', 'gr3']).state(my_state).sync()
----
...
----
envelope = pubnub.grant().channels("my_channel").read(False).write(True).\
...
----
envelope = pubnub.history().channel("my_channel").count(3).reverse(True).sync()
----
...
pubnub.delete_messages().channel(string).start(int).end(int).sync()
|===
| Parameter | Type | Required | Defaults | Description
